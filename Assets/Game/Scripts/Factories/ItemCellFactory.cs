...
namespace Game.Scripts.Factories
{
    public class ItemCellFactory
    {
        ...
        {
            var cell = _container.InstantiatePrefabForComponent<ItemCell>(_cell, container.position,
                Quaternion.identity, container);
            
            cell.Initialize(config.Type, config.Icon, config.NameRussian, config.NameEnglish, config.NameTurkish,
                degreeOfOccupancy);
