    ...
    public class MoverHorizontal : MonoBehaviour
    {
        ...
        private void FixedUpdate()
        {
            ...
            else if (Mathf.Abs(_currentDirection - _targetDirection) > _stopThreshold)
            {
                ...
                if (Mathf.Approximately(_playerBody.position.x, -RangePosition) ||
                    Mathf.Approximately(_playerBody.position.x, RangePosition))
                    _currentDirection = Mathf.Lerp(_currentDirection, _targetDirection, 0.5f);
                else
                    _currentDirection = Mathf.Lerp(_currentDirection, _targetDirection,
                        _accelerationCurve.Evaluate(tDirection));
            }
